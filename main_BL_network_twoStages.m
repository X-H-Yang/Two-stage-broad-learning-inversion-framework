...
%                       "Two-stage broad learning inversion framework for 
...
%       Comparison" and "Two-stage broad learning inversion framework for 
%       shear-wave velocity estimation"; the users can cite this paper for
%       scientific research.
% 
...
%% create the input and output datasets with 500 samples - 1st stage
disp('======================= 1st inversion stage =======================')
train_samples_N = 500;
...
%% BL network - network training and complexity selection - 1st stage
% the network complexity selection is performed based on forward modeling
disp('Performing 1st stage inversion ...')
...
disp('1st stage inversion finished ...')

disp('=================== 1st inversion stage finished ==================')

time_1st_stage = toc

Y_hat

%% draw plots - 1st stage
...
tic
%% create the input and output datasets with 500 samples - 2nd stage
Vs_profile_lower_2 = Y_hat*0.75;
Vs_profile_upper_2 = Y_hat*1.25;

disp('======================= 2nd inversion stage =======================')
train_samples_N = 500;
disp('Waiting a few tens of seconds for generating training samples ...')
% the generation of 500 samples
[train_x_2,train_y_2] = getSamples_RW_fieldData_sub_2(train_samples_N,...
    modes_num_vec,index_vec_all,f,Vp_true,den_true,Vs_profile_lower_2,...
    Vs_profile_upper_2,dispersions_R_true);
disp('Training samples were obtained!')
...
%% BL network - network training and complexity selection - 2nd stage
% the network complexity selection is performed based on forward modeling
disp('Performing 2nd stage inversion ...')
[Y_hat_2,all_index_2,NumFea_hat_2,NumWin_hat_2,NumEnhan_hat_2] = ...
    bls_regression_Y_sub_noTest(train_x_norm_2,train_y_norm_2,Fea_vec,...
    Win_vec,Enhan_vec,validation_x,Vp_true,den_true,f,modes_num_vec,...
    index_vec,index_vec_all,mean_y_2,std_y_2,validation_x_norm_2);
dispersions_R_inverted_2 = calDispersions_2(Y_hat_2,Vp_true,den_true,f,...
    modes_num_vec,index_vec_all); % inverted dispersion curves
disp('2nd stage inversion finished ...')

disp('=================== 2nd inversion stage finished ==================')

time_2nd_stage = toc

all_index_2(:,1) = all_index_2(:,1) + time_1st_stage;
...
plot(curve_02(:,1),curve_02(:,2),'k.','MarkerSize',myMarkerSize);
plot(curve_03(:,1),curve_03(:,2),'k.','MarkerSize',myMarkerSize);
drawDispersionsCompareField_5(dispersions_R_inverted_2,f,modes_num_vec,...
    index_vec,myFontSize) % inverted
axis([0 100 200 600]);
set(gca,'XTick',0:20:100);
set(gca,'YTick',100:100:700);
...
% draw objective function curve
figure(5)
plot(all_index_2(:,1),all_index_2(:,2),'b','Linewidth',1.5);
xlabel('Time [s]','FontSize',myFontSize);
ylabel('Obj','FontSize',myFontSize);
set(gca,'FontName','Times New Roman','FontSize',myFontSize);
...
% draw Vs profile
figure(6)
drawProfile_fun(Y_hat_2,h_true,Vs_true)
xlabel('Shear-wave velocity [m/s]','FontSize',myFontSize);
ylabel('Depth [m]','FontSize',myFontSize);
axis([100 800 0 25]);
set(gca,'FontName','Times New Roman','FontSize',myFontSize);
